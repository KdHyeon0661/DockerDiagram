...
namespace DockerDiagram.Helpers
{
    public sealed class KubernetesRelationshipService
    {
        ...
        public KubernetesRelationshipService(IDialogService dialogService)
        {
            ...
            IReadOnlyCollection<KubernetesNodeMetadata> all)
        {
                         ...
                         candidate.Namespace.Equals(source.Namespace, StringComparison.OrdinalIgnoreCase)))
            {
                ...
                AddConnectorIfMissing(
                    ...
                    string.Join(", ", source.SelectorLabels.Select(kv => $"{kv.Key}={kv.Value}")));
            }
        }
            ...
            string? detail)
        {
            ...
            bool exists = sheet.Connectors.Any(connector =>
            ...
            {
                RelationType = relationType,
                MountPath = relationType == RelationType.KubernetesVolumeClaim ? detail : null,
                IpAddress = relationType == RelationType.KubernetesSelector ? detail : null
            });
        }
